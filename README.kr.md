...
- **데이터 바인딩(초기값)**: `<form>` 태그에 `data-t-scope`를 지정하여 기존 데이터를 입력란 초기값으로 세팅할 수 있습니다(수정 화면 등에서 유용).
...
숫자를 '3자리 콤마 구분기호' 형식으로 출력합니다.

- **형식**: `{ 항목명 | number }`

#### `json`

데이터를 서식이 지정된 JSON 문자열로 출력합니다. 디버깅이나 JavaScript에서 데이터를 직접 사용하고 싶을 때 유용합니다.
...
서버를 시작하지 않고, 브라우저에서 `index.html`을 로컬 파일(`file://`)로 직접 열어 개발을 진행하는 모드입니다.

#### File System Access API를 통한 빌드 없는 개발

현대적인 브라우저(Chrome, Edge 등)가 제공하는 **File System Access API**를 활용함으로써, 기존에 필요했던 'JSON 파일을 JS 파일로 변환하는 빌드 프로세스'가 불필요해집니다.

1. **프로젝트 폴더 선택**: `file://`로 페이지를 열면 초기화 시 폴더 선택 프롬프트가 표시됩니다. 프로젝트의 루트 디렉토리를 선택하면 브라우저가 직접 파일을 가져오고 조작할 수 있게 됩니다.
2. **빌드 없는 미리보기**: `_sys/data/*.json`이나 `_parts/*.html`이 브라우저에 의해 직접 읽히기 때문에, 파일을 편집하고 저장한 후 브라우저를 새로고침(또는 이동)하는 것만으로 즉시 반영됩니다.

#### 페이지 이동과 SPA 라우팅

`file://` 환경에서 브라우저를 새로고침하면 폴더 액세스 권한이 초기화되기 때문에, Bracify는 모든 이동을 SPA(Single Page Application)로 처리합니다.

- **링크 자동 가로채기**: `<a>` 태그를 통한 내부 이동은 자동으로 감지되어, 페이지를 새로고침하지 않고 DOM만 교체하여 이동합니다(Full DOM Replacement).
- **JavaScript 이동 API**: 스크립트에서 프로그램 방식으로 페이지 이동을 수행하는 경우, `location.href` 대신 `Bracify.navigate('/path/to/page.html')`를 사용하십시오.
- **브라우저 히스토리**: 브라우저의 '뒤로 가기/앞으로 가기' 버튼도 지원하며, 새로고침 없이 히스토리 간 이동이 가능합니다.

#### 비지원 브라우저에서의 제한

File System Access API를 지원하지 않는 브라우저이거나 폴더 선택을 수행하지 않은 경우, 다음과 같은 제한적인 '읽기 전용 모크 모드'로 동작합니다.

- **필터 기능 제한**: 완전 일치만 가능. `:gt` 또는 `:lt` 등의 연산자는 동작하지 않습니다.
- **업데이트 불가**: 폼 전송을 통한 데이터 저장은 수행되지 않습니다.
- **부분적인 표시**: 외부 파일 읽기(`data-t-include`)가 제한될 수 있습니다.

#### 데이터 필터링 및 제어에 대하여 (공통 사양)

로컬 개발 모드(True Zero Server / 모크 모드 공통)에서는 브라우저 내에서 간이적인 데이터 처리를 수행하기 때문에 다음과 같은 제한과 사양이 있습니다.

- **필터 기능 제한**:
  - **완전 일치만 가능**: 지정된 키와 값이 완전히 일치하는 경우에만 데이터를 반환합니다.
  - **빈 값 무시**: 검색 파라미터 값이 빈 문자열(`?name=`)인 경우 해당 필터 조건 자체가 무시됩니다(모든 항목 표시).
  - **고급 연산자 비지원**: `:gt` 또는 `:lt` 등의 연산자는 기능하지 않으며 무시되거나 예상대로 동작하지 않습니다.

- **지원하는 제어 파라미터**:
  다음 파라미터는 로컬 환경에서도 간이적으로 동작합니다.
  - `_limit`: 표시 건수 제한
  - `_offset`: 데이터 건너뛰기
  - `_sort`: 정렬 대상 키
  - `_order`: `asc`(오름차순) 또는 `desc`(내림차순)

#### SPA 모드 시 JavaScript 동작

로컬 개발 모드(True Zero Server Mode)에서 페이지를 새로고침하지 않고 이동할 때, JavaScript는 다음 규칙에 따라 실행됩니다.

- **스코프 분리 (IIFE 래핑)**: 이전 페이지의 변수 선언(`const`, `let`)과의 충돌을 방지하기 위해, 페이지 고유의 스크립트(`<body>` 내 및 새로 로드된 `<head>` 내 스크립트)는 Bracify에 의해 자동으로 즉시 실행 함수(IIFE)로 래핑되어 실행됩니다.
- **중복 실행 방지**: `<head>` 내에 기술된 스크립트 중 이미 로드된 것(`engine.js`나 공통 라이브러리 등)은 이동 대상 페이지에서도 중복하여 실행되지 않습니다.
- **글로벌 변수 유지**: `window` 객체에 명시적으로 담은 데이터나 IIFE 외부에서 정의된 `var` 변수는 이동 후에도 유지됩니다.
- **이벤트 리스너**: `window`나 `document`에 직접 추가된 이벤트 리스너는 이동 시 자동으로 정리되지 않습니다. 페이지 고유의 이벤트는 가급적 `<body>` 내의 요소에 설정하거나 이동을 고려한 설계를 권장합니다.

## 데이터베이스 설정 (Database Configuration)

Bracify는 기본적으로 내장된 SQLite(`_sys/data.db`)를 사용하지만, 연결 설정을 구성하여 MySQL이나 PostgreSQL과 같은 외부 데이터베이스에 연결할 수 있습니다.

### 작동 방식

시작 시 Bracify는 항상 프로젝트 내의 `_sys/data.db`(SQLite)를 참조하여 시스템 설정을 확인합니다. 데이터베이스 연결 및 라우팅 설정은 이 파일 내의 `config` 테이블에 저장됩니다.

 이를 통해 자격 증명과 같은 민감한 정보를 텍스트 파일로 관리하거나 커밋할 필요가 없으며, 보안 정보를 저장소 외부에 유지하여 안전한 운영을 보장합니다.

### 기본 동작 (설정 없음)

`config` 테이블이 존재하지 않거나 특정 엔티티에 대한 설정이 없는 경우, **내장된 SQLite(`_sys/data.db`)가 자동으로 사용됩니다.** 간단한 프로젝트의 경우 설정이 필요하지 않습니다.

### 설정 방법

GUI(Bracify Studio)를 통해 설정하거나, 다음 형식으로 데이터베이스의 `config` 테이블에 값을 직접 삽입하여 설정할 수 있습니다.

- **대상 테이블**: `config`
- **컬럼**: `name` = 'db', `value` = (아래의 연결 정보 JSON 배열)
...
#### 라우팅 우선순위 (target_entity)

연결 대상은 다음 규칙에 따라 엔티티 이름을 기준으로 자동으로 선택됩니다.

1. **완전 일치**: 이름과 완전히 일치하는 설정이 가장 높은 우선순위를 가집니다.
2. **패턴 일치**: 와일드카드 `*`를 포함하는 설정의 경우, "고정된 부분(문자)이 가장 긴" 설정이 우선됩니다(예: `data_*`는 `*`보다 우선됨).
3. **정의 순서**: 고정된 부분의 길이가 같은 여러 패턴이 일치하는 경우, **JSON 배열에서 더 상단에 정의된** 설정이 우선됩니다.
4. **내장 SQLite**: 위의 규칙 중 어느 것도 일치하지 않는 경우 폴백으로 사용됩니다.

- **engine**: `sqlite`, `mysql`, `postgresql`, `mongodb` 등(순차적으로 구현됨).
- **option**: 드라이버별 연결 설정. `${ENV_VAR}` 형식을 사용하여 환경 변수를 지원합니다.
