...
- **지정 방법**: 임의의 명칭 지정 후 `href` 속성으로 데이터 취득 URL 지정
- **데이터 취득 URL**: `/_sys/data/{데이터 정의 명칭}.json`
  - **참고**: PC에서 확인 시 위 디렉토리에 JSON 파일을 배치하십시오. HTML 출력 샘플 데이터로 이용할 수 있습니다.
- **제약**: `<link>` 태그에만 지정 가능
- **예시**:
...
- **여러 건을 표시할 경우 (`data-t-list`)**:
    표시할 데이터가 여러 건일 경우 반복하고자 하는 범위(요소)에 `data-t-list="데이터소스명"`을 지정해야 합니다.
