...
HTMLに出力するためにデータを取得して、そのデータに名前をつけます。

- **指定方法**: 任意の名称を指定、`href`属性でデータ取得URLを指定
- **データ取得URL**: `/_sys/data/{データ定義名称}.json`
  - **補足**: PC上で確認する時は上記ディレクトリにjsonファイルを配置してください。サンプルデータとしてHTML出力に利用できます。
- **制約**: `<link>` タグのみ指定可能
- **例**:
