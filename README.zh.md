...
- **数据绑定（初始值）**：通过在 `<form>` 标签中指定 `data-t-bind`，可以将现有数据设置为输入栏的初始值（常用于编辑页面等）。
...
### 本地预览 ("Zero Server" 模式) 的数据限制

在不启动服务器，作为本地文件 (`file://`) 进行预览时（如双击 `index.html`），数据获取处理将作为浏览器内的简易 Mock 运行。
该模式旨在进行设计确认或简单的动作检查，其行为与服务器环境 (SSR) 部分不同。

- **过滤功能的限制**：
  - **仅完全一致**：仅当指定的键与值完全一致时才返回数据。
  - **忽略空值**：搜索参数值为空字符串（`?name=`）时，将忽略该过滤条件本身（展示所有记录）。
  - **不支持高级运算符**：`:gt` 或 `:lt` 等运算符不起作用，将被忽略或无法按预期工作。
